...
function onReferenceFileChange(e: Event) {
  ...
  setReferenceFile(target.files?.[0] ?? null)
}
function onReferenceDrop(e: DragEvent) {
  isDraggingReference.value = false
  setReferenceFile(e.dataTransfer?.files?.[0] ?? null)
}
      ...
      <template v-else-if="progress">
        <section class="admin-card status-card" :class="`status-${progress.submission.status}`">
          <div class="status-row">
            ...
            <span v-if="(isForBoardApproval || isBoardApproved) && progress.submission.presidentEndorsedAt" class="muted">Endorsed by President {{ formatDateTime(progress.submission.presidentEndorsedAt) }}</span>
        ...
        <section class="admin-card">
          <div v-if="isForPresidentReference" class="screening-actions">
            ...
            <label
              class="file-drop"
              :class="{ dragging: isDraggingReference, filled: !!referenceFile }"
              @dragover.prevent="isDraggingReference = true"
              @dragleave.prevent="isDraggingReference = false"
              @drop.prevent="onReferenceDrop"
            >
              <input type="file" accept=".pdf,.jpg,.jpeg,.png,.doc,.docx" hidden @change="onReferenceFileChange" />
              <template v-if="referenceFile">
                <div class="file-drop-chip">
                  <span class="file-drop-icon">📄</span>
                  <div class="file-drop-info">
                    <span class="file-drop-name">{{ referenceFile.name }}</span>
                    <span class="file-drop-size">{{ formatFileSize(referenceFile.size) }}</span>
                  </div>
                  <button type="button" class="file-drop-remove" @click.prevent.stop="clearReferenceFile">✕</button>
                </div>
              </template>
              <template v-else>
                <span class="file-drop-icon">⬆</span>
                <span class="file-drop-text"><strong>Click to choose a file</strong> or drag it here</span>
                <span class="file-drop-hint">PDF, JPG, PNG, or DOC — the signed reference slip</span>
              </template>
            </label>

            <div class="action-buttons">
              <button class="action-btn primary" :disabled="referring || !referenceFile" @click="onSubmitReference">
